...
export function AppProviders({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isFirstLaunch && setupComplete) {
      if (connectionMode === "local") {
        // This build is local-only: there is no sign-in flow, so there is no
        // session to detect and nothing to upgrade to. Stay local.
        setAuthChecked(true);
      } else {
        authService
          .isAuthenticated()
  ...
  useEffect(() => {
    ...
    // Sign-in prompt removed in this build — clear the flag and carry on.
  ...
  return (
    ...
    >
      <ToolActionsContext.Provider
        value={{
          // No sign-in flow in this build. Left undefined rather than a no-op:
          // OperationButton keys its tooltip and pointer cursor off this being
          // set, so a no-op would still advertise "Click to sign in".
